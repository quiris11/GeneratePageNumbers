...
class PdbHeaderReader:
    ...
    def name(self):
        self.stream.seek(0)
        return re.sub(b'[^-A-Za-z0-9 ]+', b'_', self.stream.read(
            32).replace(b'\x00', b''))
...
class PdbHeaderBuilder:
    ...
    def __init__(self, identity, title):
        ...
        self.title = b'%s\x00' % re.sub(
            b'[^-A-Za-z0-9 ]+', b'_', title).ljust(31, b'\x00')[:31]
    ...
    def build_header(self, section_lengths, out_stream):
        ...
        out_stream.write(self.title + struct.pack(
            '>HHIIIIII', 0, 0, now, now, 0, 0, 0, 0))
        out_stream.write(self.identity + struct.pack(
            '>IIH', nrecords, 0, nrecords))

        offset = 78 + (8 * nrecords) + 2
        for id, record in enumerate(section_lengths):
            out_stream.write(
                struct.pack('>LBBBB', int(offset), 0, 0, 0, 0))
            offset += record
