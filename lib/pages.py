...
def find_exth(search_id, content):
    exth_begin = content.find(b'EXTH')
...
def get_pages(dirpath, mfile, is_verbose):
    ...
    if mobi_content[60:68] != b'BOOKMOBI':
        print(mfile + ': invalid file format. Skipping...')
        return None
    id, ver, title, locations, di, do = mobi_header_fields(mobi_content)
    if (di != 0 or do != 0):
        if is_verbose:
            print(mfile + ': dictionary file. Skipping...')
        return None
    ...
    if (asin == '* NONE *'):
        asin = b'* NONE *'
    if b'!DeviceUpgradeLetter!' in asin:
        if is_verbose:
            print(mfile + ': Upgrade Letter. Skipping...')
        return None
    row = [
        asin.decode('utf-8'),
        dc_lang.decode('utf-8'),
        author.decode('utf-8'),
        title.decode('utf-8'),
        int(locations / 15 + 1),
        False,
        os.path.join(mfile).encode('utf-8').decode('utf-8')
    ]
    # print(row)
    return row
